#!/usr/bin/env python3
"""Evaluate a static PTQ checkpoint over FlashVSR validation clips vs reusable FP16 refs."""
...
RUN = ROOT / os.environ["STATIC_PTQ_RUN"]
CKPT = ROOT / os.environ["STATIC_PTQ_CKPT"]
TAG = os.environ.get("STATIC_PTQ_TAG", "static_ptq")
...
def main() -> None:
    ...
    for clip in CLIPS:
        ...
        if not fp16.exists() or fp16.stat().st_size == 0:
            raise FileNotFoundError(fp16)
        mixed = VIDEOS / f"{clip}_{TAG}_first16.mp4"
        common = [
        ...
        if not mixed.exists() or mixed.stat().st_size == 0:
            run(common + ["--output", str(mixed), "--quantize_mode", "FakeQuant_A8W8", "--ckpt_path", str(CKPT)], REPORTS / f"{clip}_{TAG}.log")
        psnr_json = METRICS / f"{clip}_psnr_fp16_vs_{TAG}_first16.json"
