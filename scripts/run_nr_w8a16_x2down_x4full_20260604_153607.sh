...
RUN_ID=20260604_153607_flashvsr_w8a16_nr_x2down_x4full_first3s
OUT_ROOT=/home/user/SynologyDrive/daily/${RUN_ID}
CKPT=models/FlashVSR-v1.1/diffusion_pytorch_model_w8a16.safetensors

cd "$REPO"
mkdir -p "$OUT_ROOT" "$OUT_ROOT/trimmed_3s" "$OUT_ROOT/downsample_x2_bicubic_antialias" "$OUT_ROOT/flashvsr_x4_w8a16_full" "$OUT_ROOT/logs" "$OUT_ROOT/tmp"
...
PY
  ...
  OUT="$OUT_ROOT/flashvsr_x4_w8a16_full/${SAFE}_first3s_x2down_flashvsr_x4_w8a16_full.mp4"
...
PY
  ...
  echo "[3/3] FlashVSR W8A16 mode=full scale=4 -> $OUT"
  python cli_main.py \
    --input "$LR" \
    --output "$OUT" \
    --model FlashVSR-v1.1 \
...
from pathlib import Path
import json
root = Path('/home/user/SynologyDrive/daily/20260604_153607_flashvsr_w8a16_nr_x2down_x4full_first3s')
lines = [
    ...
    '- VRAM safety: CLI ran with `--tiled_vae --tiled_dit --tile_size 512 --tile_overlap 32`',
    '',
    '## Artifacts',
    f'- Root: `{root}`',
    f'- Manifest: `{root / "manifest.tsv"}`',
    f'- Downsample videos: `{root / "downsample_x2_bicubic_antialias"}`',
    f'- FlashVSR outputs: `{root / "flashvsr_x4_w8a16_full"}`',
